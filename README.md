...
- `src/client/logo-row.svg` — the row form, used by the boot splash (inlined into the shell at build time; draws in `currentColor` so it follows the theme). Inside the app the brand is the text "CUPI Wiki" in the display serif (Playfair Display) at the top of the sidebar; the mark itself is not shown there.
