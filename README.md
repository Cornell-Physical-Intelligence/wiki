...
- **Website feed.** `GET /api/recruit/site` (public, CORS-limited to the site, never cached) describes the cycle receiving the website and its sections: key, title, description, `open`, and the question list (`short`, `long`, `email`, `single`, `multi`, `checkbox`, `link`, `file`). `POST /api/recruit/site/<section>` takes `{ answers, files, website, confirmUpdate }`, validates against that section's form, journals the receipt, writes the row, and emails the team when the cycle asks for it. `POST /api/interest` is unchanged and lands in the receiving cycle's interest section. Before any cycle receives the website, the feed publishes the fixed interest form and the fixed POST goes to the legacy inbox as before.
- **Editing forms.** Cycle → Settings → Website sections: per section, open or closed, title, description, and the questions (label, type, options, required, order). The interest form keeps the website's six fixed questions (name, email, subteam, year, project, file) and can gain more; the other two only need a name and an email. The website reads the change on its next load.
