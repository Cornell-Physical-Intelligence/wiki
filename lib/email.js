...
let FROM_CACHE = null;
export async function resolveFrom(apiKey, settings) {
...
export async function sendWelcome({ to, addedByName, host, settings, clientId, saveOauth }) {
  ...
  // Delivery lives in lib/email-send.js, shared with the recruitment mailer;
  // the request body is the same as before the split. Loaded lazily because
  // that module imports the credential helpers from here.
  const { sendEmail } = await import('./email-send.js');
  const { id, ...out } = await sendEmail({ to, subject: "You're on the CUPI wiki", html, settings, clientId, saveOauth });
  return out;
}
