...
export async function sendWelcome({ to, addedByName, host, settings }) {
  const apiKey = settings?.key || process.env.RESEND_API_KEY;
  if (!apiKey) return { sent: false, reason: 'no Resend API key yet. An admin can add one under Members & access → Email' };
