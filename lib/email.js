...
// One notification per new interest-form submission. Same transport and
// degradation story as the welcome email: without a Resend connection the
// submission is still stored; the email is the courtesy copy.
const escHtml = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

export async function sendInterestNotice({ to, sub, host, settings, clientId, saveOauth }) {
  let apiKey;
  ...
  <div style="font-family:system-ui,-apple-system,'Segoe UI',sans-serif;max-width:520px;margin:0 auto;padding:0 4px;color:#141414;background:#ffffff">
    ...
    <table style="border-collapse:collapse">
      ${row('Name', `<b>${escHtml(sub.name)}</b>${sub.cornell ? '' : ' <span style="color:#8a5f00;font-size:12px">(not a cornell.edu address)</span>'}`)}
      ${row('Email', escHtml(sub.email))}
      ${row('Subteam', escHtml(sub.subteam || 'Not sure yet'))}
      ${row('Coolest project', sub.project ? escHtml(sub.project) : '<span style="color:#999">(blank)</span>')}
      ${row('File', sub.fileName ? `${escHtml(sub.fileName)} (${Math.max(1, Math.round(sub.fileSize / 1024))} KB, in the wiki)` : '<span style="color:#999">none</span>')}
    </table>
    <p style="margin:22px 0"><a href="${url}/#/admin" style="display:inline-block;background:#141414;color:#ffffff;text-decoration:none;padding:10px 20px;border-radius:6px;font-weight:600">Open the interest list</a></p>
  ...
  try {
    ...
    if (!r.ok) {
      ...
      if (/testing emails|verify a domain|not verified|not authorized to send emails from/i.test(detail)) {
        return { sent: false, reason: `Resend refuses the From address (${from}). Pick an address on a verified domain under Members & access → Email` };
      }
