...
async function siteRoute(rq, kit) {
  ...
  await Promise.all(sections.map(async (s) => {
    const definition = sectionsFor(cycle)[s.key];
    const count = definition.capacity ? await kit.apps.count(cycle.id, s.key, true) : 0;
    s.full = Boolean(definition.capacity && count >= definition.capacity);
    s.available = s.open && cycle.status === 'open' && !s.full;
  }));
  return { status: 200, body: { cycle: { id: cycle.id, name: cycle.name, term: cycle.term, status: cycle.status }, landing: landingFor(cycle), sections }, headers };
}
...
async function submitRoute(rq, kit) {
  ...
  const now = kit.now();
  const files = v.files;
  const intake = { perIpHour: 5, perDay: 2000, ...(cycle.doc?.intake || {}) };
  // Every rule is the form's own: its cap, whether repeats replace, who hears.
  const replaceable = section.replace !== false;
  const entry = {
  ...
  let journaled = false;
  try { journaled = await journal.append(entry, files, { perDay: intake.perDay, perIpHour: intake.perIpHour }); }
  catch (e) { if (e.status === 429) throw fail(429, e.message); }
  let result;
  try {
    result = await kit.apps.commitIntake(cycle, entry, journal, files, { confirmUpdate: entry.confirmUpdate && replaceable, source: 'site' });
  } catch (e) {
  ...
  if (result.outcome === 'held') throw fail(409, `${section.title} is full`);
  ...
  if (result.outcome === 'superseded') throw fail(409, 'A newer submission was saved. Your changes were not applied. Review them and submit again.', { code: 'SUBMISSION_SUPERSEDED' });
  if (result.outcome !== 'saved') throw fail(503, 'Could not confirm your submission. Try again in a minute.', { code: 'INTAKE_UNAVAILABLE' });
  if (result.outcome === 'saved' && result.inserted && result.row) {
    ...
    if (section.notify !== false) { try { await notifyLeads(kit, cycle, key, section, result.row); } catch (e) { /* never fails a submission */ } }
  }
