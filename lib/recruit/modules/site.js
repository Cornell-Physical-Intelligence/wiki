...
import { sectionsFor, publicSections, validateSite, SECTION_LABELS } from '../sections.js';
...
async function siteRoute(rq, kit) {
  ...
  if (!cycle) {
    ...
    return { status: 200, body: { cycle: null, sections: [{ ...interest, open: true }] }, headers };
  }
  return { status: 200, body: { cycle: { id: cycle.id, name: cycle.name, term: cycle.term, status: cycle.status }, sections: publicSections(cycle) }, headers };
}

async function notifyLeads(kit, cycle, key, section, row) {
  const settings = await kit.cycles.settings();
  const to = Array.isArray(settings.doc?.notify) ? settings.doc.notify.filter(Boolean) : [];
...
async function submitRoute(rq, kit) {
  ...
  if (!section?.open) throw fail(409, `${section?.title || SECTION_LABELS[key]} is closed right now`);
  ...
  const capacity = Number(cycle.doc?.capacity || 0);
  ...
  let result;
  try {
    result = await kit.apps.commitIntake(cycle, entry, journal, file, { confirmUpdate: entry.confirmUpdate && intake.confirmUpdate !== false, source: 'site' });
  } catch (e) {
  ...
  if (result.outcome === 'review') {
    return { status: 409, body: { exists: true, submitted: result.existing?.ts || null, receipt: entry.id, error: 'You already sent this form with this email. Confirm to replace your earlier answers.' } };
  }
  ...
  if (result.outcome === 'saved' && result.inserted && result.row) {
    ...
    if (intake.notify !== false) { try { await notifyLeads(kit, cycle, key, section, result.row); } catch (e) { /* never fails a submission */ } }
  }
