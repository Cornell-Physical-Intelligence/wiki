...
function makeFacade(kit) {
  ...
  return { get, review, ensure, reviews, updateReview, forget, seedFrom, result: (rq) => peopleResult(rq, kit) };
}
...
export async function peopleResult(rq, kit) {
  const qy = rq.query;
  const out = await kit.apps.people(rq.cycle.id, { q: qy.q, grantSubteams: rq.grant?.subteams || null, scope: rq.scope, cycle: rq.cycle });
  ...
  return { ...out, rows, total: rows.length, counts: { ...out.counts, flagged: rows.filter((p) => p.flagged).length } };
}
...
async function listRoute(rq, kit) {
  ...
  if (rq.query.cursor) {
    ...
    if (!Number.isFinite(cursor?.last) || typeof cursor.email !== 'string') throw fail(400, 'Invalid people cursor');
  }
  const eligible = cursor ? out.rows.filter((p) => p.last < cursor.last || p.last === cursor.last && p.email.localeCompare(cursor.email) > 0) : out.rows;
  const rows = eligible.slice(0, limit), last = rows.at(-1);
  const next = eligible.length > limit ? Buffer.from(JSON.stringify({last:last.last,email:last.email})).toString('base64url') : null;
  return { status: 200, body: { ...out, rows, next } };
}
...
async function csvRoute(rq, kit) {
  const out = await peopleResult({ ...rq, query: {} }, kit);
