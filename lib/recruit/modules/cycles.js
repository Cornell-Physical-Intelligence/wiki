...
const ADMIN_SECTIONS = new Set(['modules', 'intake']);
...
export function defaultDoc(modules, cycle = {}) {
  return {
    ...
    intake: { perIpHour: 5, perDay: 2000 },
...
function settingsSeed(modules) {
  ...
  return {
    intakeCycleId: null, migration: null,
...
function makeFacade(kit) {
  ...
  const facade = {
    ...
    async get(id) {
      if (!id) return null;
      if (kit.mode === 'memory') return this.settle(labelled(clone(kit.mem.cycles.find((c) => c.id === id) || null)));
      const s = await kit.sql();
      const r = await s`SELECT * FROM recruit_cycles WHERE id = ${id}`;
      return r.rows[0] ? this.settle(cycleFromPg(r.rows[0]), r.rows[0]) : null;
    },

    async list({ all = false, ids = null } = {}) {
      if (kit.mode === 'memory') {
        return kit.mem.cycles.filter((c) => (all || c.status !== 'archived') && (!ids || ids.includes(c.id)))
          .sort((a, b) => b.updated - a.updated).map((c) => this.settle(labelled(clone(c))));
      }
      const s = await kit.sql();
      const q = kit.build();
      ...
      return (await q.run(s)).rows.map((r) => this.settle(cycleFromPg(r), r));
    },

    // A cycle still stored under the placeholder label is renamed in place
    // the first time it is read, so the placeholder never comes back from
    // any other path. Idempotent; a failure only delays it to the next read.
    settle(cycle, raw = null) {
      if (!cycle) return cycle;
      const placeholder = raw ? raw.name === 'Interest list' && raw.term === 'Rolling' : false;
      if (kit.mode === 'memory') {
        const c = kit.mem.cycles.find((x) => x.id === cycle.id);
        if (c && c.name === 'Interest list' && c.term === 'Rolling') { c.name = cycle.name; c.term = cycle.term; kit.memSave(); }
        return cycle;
      }
      if (placeholder) {
        kit.sql().then((s) => s`UPDATE recruit_cycles SET name = ${cycle.name}, term = ${cycle.term} WHERE id = ${cycle.id} AND name = 'Interest list' AND term = 'Rolling'`).catch(() => {});
      }
      return cycle;
    },
    ...
    async pointer() {
      return kit.cached('cycles.pointer', TARGET_TTL, async () => {
        ...
        const s = await kit.sql();
        const r = await s`SELECT s.doc->'migration' AS migration, s.doc->>'intakeCycleId' AS intake_cycle_id, c.id, c.form_version, c.status, c.doc->'intake' AS intake
        ...
        return {
          ...
          cycle: row.id ? { id: row.id, formVersion: Number(row.form_version || 0), status: row.status, doc: { intake: asObject(row.intake) } } : null,
        };
      });
    },
    ...
    async intakeTarget() {
      ...
      const intake = { perIpHour: 5, perDay: 2000, ...(p.cycle.doc?.intake || {}) };
      return { cycleId: p.cycle.id, formVersion: p.cycle.formVersion || 0, perIpHour: intake.perIpHour, perDay: intake.perDay };
    },
    ...
    async update(id, version, { name, term, opensAt, closesAt, status, closedAt, docPatch, docSet, emptySections = [], by, now }) {
      if (kit.mode === 'memory') {
        const c = kit.mem.cycles.find((x) => x.id === id);
        if (!c || c.version !== version) return null;
        for (const key of emptySections) if (kit.mem.applications.some((a) => a.cycleId === id && (a.section || 'interest') === key)) throw fail(409, 'This form has responses. Close it instead of removing it.');
        ...
        kit.memSave();
        kit.uncache();
        return labelled(clone(c));
      }
      const sql = await kit.sql();
      const run = async (s) => {
      if (emptySections.length) {
        await s`SELECT doc FROM recruit_cycles WHERE id = ${id} FOR UPDATE`;
        const rows = await s`SELECT count(*)::int AS n FROM recruit_applications WHERE cycle_id = ${id} AND section = ANY(${emptySections})`;
        if (Number(rows.rows[0]?.n)) throw fail(409, 'This form has responses. Close it instead of removing it.');
      }
      const q = kit.build();
      ...
      return emptySections.length ? sql.transaction(run) : run(sql);
    },
...
async function listCycles(rq, kit) {
  ...
  const rows = cycles.map((c) => ({
    ...
    sections: Object.entries(sectionsFor(c)).map(([key, s]) => ({ key, title: s.title })),
...
async function patchCycle(rq, kit) {
  ...
  const docPatch = validateSettings(body.description !== undefined ? { description: body.description } : {});
...
async function putSettings(rq, kit) {
  ...
  if (target) {
    ...
    // A form with responses is closed, not removed.
    for (const key of Array.isArray(body.settings?.remove) ? body.settings.remove : []) {
      const n = await kit.apps.count(rq.cycle.id, String(key));
      if (n > 0) throw fail(409, `That form has ${n} ${n === 1 ? 'response' : 'responses'}. Close it instead of removing it.`);
    }
  ...
  const cycle = await kit.cycles.update(rq.cycle.id, version, { by: rq.me.email, now: kit.now(), emptySections: name === 'site' && Array.isArray(body.settings?.remove) ? body.settings.remove : [], ...(docPatch ? { docPatch } : { docSet: { key: name, value } }) });
  if (!cycle) { const cur = await kit.cycles.get(rq.cycle.id); throw fail(409, 'This cycle changed. Reload.', { version: cur?.version ?? null }); }
  kit.uncache();
