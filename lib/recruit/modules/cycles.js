...
// Legacy interest-list recipients come from the environment alone; each
// form names its own recipients now.
const NOTIFY = (process.env.INTEREST_NOTIFY || '').split(',').map((s) => s.trim()).filter(Boolean);
...
async function getCycle(rq, kit) {
  ...
  return {
    ...
    body: {
      ...
      sections, ...(roles ? { roles, notify: Array.isArray(settings.doc.notify) ? settings.doc.notify : [] } : {}), counts, me: { roles: rq.role === 'admin' ? ['admin'] : (rq.grant?.roles || []) },
    },
  };
}
