...
import { DEFAULT_SUBTEAMS, slug, fixedFormFor } from '../fixed-form.js';
import { importLegacy, adoptOrphans, migrationState, LIVE_CYCLE } from '../migrate.js';
...
const KERNEL_SECTIONS = new Set(['modules', 'intake', 'subteams']);
...
function settingsSeed(modules) {
  ...
  return {
    ...
    defaults: {
      stages: d.pipeline?.stages ?? null, subteams: DEFAULT_SUBTEAMS.map((s) => ({ ...s })),
      rubric: d.review?.rubric ?? null, templates: d.comms?.templates ?? {}, interviews: d.interviews?.rounds ?? null,
    },
  };
}
...
function makeFacade(kit) {
  ...
  const facade = {
    ...
    async get(id) {
      if (!id) return null;
      if (kit.mode === 'memory') return clone(kit.mem.cycles.find((c) => c.id === id) || null);
      const s = await kit.sql();
    ...
    async list({ all = false, ids = null } = {}) {
      if (kit.mode === 'memory') {
        return kit.mem.cycles.filter((c) => (all || c.status !== 'archived') && (!ids || ids.includes(c.id)))
          .sort((a, b) => b.updated - a.updated).map(clone);
      }
      const s = await kit.sql();
      const q = kit.build();
      ...
      return (await q.run(s)).rows.map(cycleFromPg);
    },
    ...
    async counts(cycleIds = null) {
      const out = {};
      const add = (cycleId, stage, n) => { const c = (out[cycleId] ||= { total: 0, byStage: {} }); c.total += n; c.byStage[stage] = (c.byStage[stage] || 0) + n; };
      if (kit.mode === 'memory') {
        for (const a of kit.mem.applications) if (!cycleIds || cycleIds.includes(a.cycleId)) add(a.cycleId, a.stage, 1);
        return out;
      }
      const s = await kit.sql();
      const q = kit.build();
      q`SELECT cycle_id, stage, count(*)::int AS n FROM recruit_applications WHERE true`;
      if (cycleIds) q` AND cycle_id = ANY(${cycleIds})`;
      q` GROUP BY cycle_id, stage`;
      for (const r of (await q.run(s)).rows) add(r.cycle_id, r.stage, Number(r.n));
      return out;
    },
...
function docFor(cycle, role) {
  ...
  const doc = { ...cycle.doc };
  delete doc.intake;
  if (isObject(doc.comms)) { doc.comms = { ...doc.comms }; delete doc.comms.templates; }
  return doc;
}

async function formFor(kit, cycle) {
  if (cycle.formVersion > 0 && kit.tables.has('recruit_forms')) {
    if (kit.mode === 'memory') {
      const f = (kit.mem.forms || []).find((x) => x.cycleId === cycle.id && Number(x.version) === cycle.formVersion);
      if (f) return { version: cycle.formVersion, doc: f.doc };
    } else {
      const s = await kit.sql();
      const r = await s`SELECT doc FROM recruit_forms WHERE cycle_id = ${cycle.id} AND version = ${cycle.formVersion}`;
      if (r.rows[0]) return { version: cycle.formVersion, doc: asObject(r.rows[0].doc) };
    }
  }
  return { version: 0, doc: fixedFormFor(cycle) };
}
...
async function listCycles(rq, kit) {
  ...
  const rows = cycles.map((c) => ({
    ...
    counts: counts[c.id] || { total: 0, byStage: {} }, updated: c.updated, version: c.version,
...
async function getCycle(rq, kit) {
  ...
  const counts = (await kit.cycles.counts([cycle.id]))[cycle.id] || { total: 0, byStage: {} };
  const form = await formFor(kit, cycle);
  ...
  return {
    ...
    body: {
      ...
      form, ...(roles ? { roles } : {}), counts, me: { roles: rq.role === 'admin' ? ['admin'] : (rq.grant?.roles || []) },
    },
  };
}
