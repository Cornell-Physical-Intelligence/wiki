...
import { sectionsFor, SECTION_KEYS } from '../sections.js';
...
function makeFacade(kit) {
  ...
  async function history(email, exceptId) {
    if (kit.mode === 'memory') {
      ...
      return memApps().filter((a) => a.email === email && a.id !== exceptId && a.erasedAt == null).sort((a, b) => b.ts - a.ts).slice(0, 20)
        .map((a) => ({ id: a.id, cycleId: a.cycleId, cycleName: cycles.get(a.cycleId) || a.cycleId, section: sectionOf(a), stage: a.stage, outcome: a.outcome, ts: a.ts }));
    }
    const s = await kit.sql();
    const r = await s`SELECT a.id, a.cycle_id, c.name AS cycle_name, a.section, a.stage, a.outcome, a.ts FROM recruit_applications a LEFT JOIN recruit_cycles c ON c.id = a.cycle_id
      WHERE a.email = ${email} AND a.id <> ${exceptId} AND a.erased_at IS NULL ORDER BY a.ts DESC LIMIT 20`;
    return r.rows.map((x) => ({ id: x.id, cycleId: x.cycle_id, cycleName: x.cycle_name || x.cycle_id, section: x.section || 'interest', stage: x.stage, outcome: x.outcome || null, ts: Number(x.ts) }));
  }
  ...
  return {
    get, findByEmail, count, exportRows, ids, commitIntake, updateReview, move, setDecision, patch, remove, toLegacyRow, legacyRows, create, addFile, fileOwner,
    applicant, history, people, eraseApplicant, purgePage, receiptOutcome, recordOutcome, listRow, list,
  };
}
...
async function listRoute(rq, kit) {
  ...
  const out = await kit.apps.list(rq.cycle, {
    q: qy.q, section: qy.section, stage: qy.stage, subteam: qy.subteam, year: qy.year, tag: qy.tag, flagged: qy.flagged, outcome: qy.outcome, source: qy.source,
...
async function peopleRoute(rq, kit) {
  return { status: 200, body: await kit.apps.people(rq.cycle.id, { q: rq.query.q, grantSubteams: rq.grant?.subteams || null, cycle: rq.cycle }) };
}
...
async function detailRoute(rq, kit) {
  ...
  const section = sectionsFor(rq.cycle)[app.section || 'interest'];
  const form = { questions: (section || sectionsFor(rq.cycle).interest).form.questions };
...
async function csvRoute(rq, kit) {
  const key = SECTION_KEYS.includes(rq.query.section) ? rq.query.section : 'interest';
  const section = sectionsFor(rq.cycle)[key];
  ...
  const columns = [
    ...
    ...section.form.questions.filter((q) => !['name', 'email', 'subteam', 'year'].includes(q.key) && q.type !== 'file').map((q) => ({ header: q.label || q.key, cell: (r) => safe(Array.isArray(r.answers?.[q.key]) ? r.answers[q.key].join('; ') : r.answers?.[q.key] ?? '') })),
    { header: 'Files', cell: (r) => safe((r.files || []).map((f) => f.name).join('; ')) },
    { header: 'Flagged', cell: (r) => (r.review?.flagged ? 'yes' : '') }, { header: 'Comments', cell: (r) => (r.review?.comments || []).length },
  ];
...
export default {
  ...
  routes: [
    ...
    { method: 'GET', path: '/cycles/:cycle/people', access: 'role', scoped: true, handler: peopleRoute },
