...
import { validateAnswers, DEFAULT_STAGES, FILE_TYPES, MAX_FILE, EMAIL_RE, isCornell, slug, COLUMN_KEYS } from '../fixed-form.js';
import { sectionsFor, formForRow } from '../sections.js';
import { roleOf, scopeFor, visibleHistory } from '../permissions.js';
...
function makeFacade(kit) {
  ...
  async function count(cycleId, section = null, activeOnly = false) {
    if (kit.mode === 'memory') return memApps().filter((a) => a.cycleId === cycleId && (!section || sectionOf(a) === section) && (!activeOnly || a.erasedAt == null)).length;
    const s = await kit.sql();
    const r = section
      ? await s`SELECT count(*) AS n FROM recruit_applications WHERE cycle_id = ${cycleId} AND section = ${section} AND (${!activeOnly}::boolean OR erased_at IS NULL)`
      : await s`SELECT count(*) AS n FROM recruit_applications WHERE cycle_id = ${cycleId} AND (${!activeOnly}::boolean OR erased_at IS NULL)`;
    return Number(r.rows[0]?.n || 0);
  }

  // Every row of one section, newest first, for the CSV export.
  async function exportRows(cycleId, section) {
    if (kit.mode === 'memory') return memApps().filter((a) => a.cycleId === cycleId && sectionOf(a) === section).sort((a, b) => b.ts - a.ts).map(project);
    const s = await kit.sql();
    const r = await s`SELECT * FROM recruit_applications WHERE cycle_id = ${cycleId} AND section = ${section} ORDER BY ts DESC`;
    return r.rows.map(appFromPg);
  }
  ...
  async function ids(cycleId, { subteams = null, cycle = null } = {}) {
    ...
    if (kit.mode === 'memory') return new Set(memApps().filter((a) => a.cycleId === cycleId && (!names || names.includes(a.subteam) || (Array.isArray(a.answers?.subteam) && a.answers.subteam.some((v) => names.includes(v))))).map((a) => a.id));
    const s = await kit.sql();
    const q = kit.build();
    q`SELECT id FROM recruit_applications WHERE cycle_id = ${cycleId}`;
    if (names) q` AND (subteam = ANY(${names}) OR (jsonb_typeof(answers->'subteam') = 'array' AND answers->'subteam' ?| ${names}))`;
  ...
  async function commitIntake(cycle, entry, journal, originalFile = null, { confirmUpdate = Boolean(entry.confirmUpdate), override = false, source = 'form' } = {}) {
    ...
    const form = sectionsFor(cycle)[section];
    if (!form) throw fail(409, 'This cycle no longer has the receipt’s form');
    if (form.replace === false && !override) confirmUpdate = false;
    const modern = entry.version === 2;
    const supplied = Array.isArray(originalFile) ? originalFile : originalFile ? [originalFile] : [];
    const manifest = Array.isArray(entry.files) ? entry.files : entry.fileSize ? [{ question: 'file', name: entry.fileName, type: entry.fileType, size: entry.fileSize }] : [];
    const fileEntries = [], fileWrites = [];
    for (const [index, attachment] of manifest.entries()) {
      const data = supplied[index]?.data || await journal.getFile(entry.id, Array.isArray(entry.files) ? index : null);
      if (!data || data.length !== attachment.size) throw new Error('Saved attachment is not available yet');
      const fileId = 'int-' + stripId(entry.id) + (Array.isArray(entry.files) ? 'f' + index : '');
      fileWrites.push({ id: fileId, name: attachment.name, type: attachment.type, size: data.length, by: email, ts: entry.ts, data });
      fileEntries.push({ id: fileId, question: attachment.question, name: attachment.name, type: attachment.type, size: data.length });
    }
    if (kit.mode === 'memory') for (const file of fileWrites) await kit.files.put(file);
    // An omitted upload is not a deletion request. Replace only the questions
    // with new files, keeping the other attachments across applicant updates.
    const mergeFiles = (previous = []) => modern
      ? [...previous.filter((f) => !fileEntries.some((next) => (next.question || 'file') === (f.question || 'file'))), ...fileEntries]
      : fileEntries.length ? fileEntries : previous;
    ...
    const row = {
      ...
      answers: modern ? answers : { ...(existing?.answers || {}), ...answers }, files: mergeFiles(existing?.files),
    ...
    if (kit.mode === 'memory') {
      ...
      const currentForm = sectionsFor(kit.mem.cycles.find((c) => c.id === cycle.id) || cycle)[section];
      if (!currentForm) throw fail(409, 'This cycle no longer has the receipt’s form');
      if (currentForm.replace === false && !override) confirmUpdate = false;
      const capacity = currentForm.capacity || 0;
      if (!current && capacity && memApps().filter((a) => a.cycleId === cycle.id && sectionOf(a) === section && a.erasedAt == null).length >= capacity) outcome = 'held';
      else if (current && !confirmUpdate) outcome = 'review';
        ...
        if (current) {
          ...
          Object.assign(current, {
            ...
            year: entry.hasYear ? (entry.year || null) : current.year, answers: modern ? answers : { ...(current.answers || {}), ...answers },
            files: mergeFiles(current.files), receiptId: rid,
          });
    ...
    const sql = await kit.sql();
    return sql.transaction(async (s) => {
    // Serialize admissions for a cycle. A second statement after acquiring
    // this lock sees the preceding transaction's committed insert.
    const locked = await s`SELECT doc FROM recruit_cycles WHERE id = ${cycle.id} FOR UPDATE`;
    const currentForm = sectionsFor({ ...cycle, doc: asObject(locked.rows[0]?.doc) })[section];
    if (!locked.rows.length || !currentForm) throw fail(409, 'This cycle no longer has the receipt’s form');
    if (currentForm.replace === false && !override) confirmUpdate = false;
    if (currentForm.capacity) {
      const admitted = await s`SELECT count(*)::int AS n, count(*) FILTER (WHERE email = ${email})::int AS own
        FROM recruit_applications WHERE cycle_id = ${cycle.id} AND section = ${section} AND erased_at IS NULL`;
      if (!Number(admitted.rows[0]?.own) && Number(admitted.rows[0]?.n) >= currentForm.capacity) {
        await s`INSERT INTO interest_receipts (id, outcome, ts) VALUES (${rid}, 'held', ${entry.ts}) ON CONFLICT (id) DO NOTHING`;
        return { outcome: 'held', inserted: false };
      }
    }
    const r = await s`WITH prior AS (
      ...
      SELECT ${row.id}, ${row.cycleId}, ${section}, ${row.email}, ${row.ts}, ${row.updated}, ${row.name}, ${row.cornell}, ${row.subteam}, ${row.year}, ${row.source}, ${row.formVersion},
        ${JSON.stringify(answers)}::jsonb, ${JSON.stringify(fileEntries)}::jsonb, ${row.stage}, ${row.ts}, ${JSON.stringify(row.stageHistory)}::jsonb, ${row.ipHash}, ${rid}
      ...
      ON CONFLICT (cycle_id, section, email) DO UPDATE SET name = EXCLUDED.name, subteam = EXCLUDED.subteam, updated = EXCLUDED.updated, ip_hash = EXCLUDED.ip_hash,
        ...
        answers = CASE WHEN ${modern} THEN EXCLUDED.answers ELSE recruit_applications.answers || EXCLUDED.answers END,
        files = CASE WHEN ${modern} THEN
          COALESCE((SELECT jsonb_agg(old.file ORDER BY old.ord)
            FROM jsonb_array_elements(recruit_applications.files) WITH ORDINALITY AS old(file, ord)
            WHERE NOT EXISTS (SELECT 1 FROM jsonb_array_elements(EXCLUDED.files) AS incoming(file)
              WHERE COALESCE(incoming.file->>'question', 'file') = COALESCE(old.file->>'question', 'file'))), '[]'::jsonb) || EXCLUDED.files
          WHEN jsonb_array_length(EXCLUDED.files) > 0 THEN EXCLUDED.files ELSE recruit_applications.files END,
    ...
    if (outcome === 'saved') for (const file of fileWrites) await kit.files.put(file, s);
    const savedRows = outcome === 'saved' ? await s`SELECT * FROM recruit_applications WHERE cycle_id = ${cycle.id} AND section = ${section} AND email = ${email}` : { rows: [] };
    const saved = savedRows.rows[0] ? appFromPg(savedRows.rows[0]) : null;
  ...
  async function history(email, exceptId) {
    ...
    const r = await s`SELECT a.id, a.cycle_id, c.name AS cycle_name, a.section, a.ts FROM recruit_applications a LEFT JOIN recruit_cycles c ON c.id = a.cycle_id
      WHERE a.email = ${email} AND (${exceptId}::text IS NULL OR a.id <> ${exceptId}) AND a.erased_at IS NULL ORDER BY a.ts DESC LIMIT 20`;
  ...
  // section. Newest activity first. Routes paginate the grouped people;
  // exports include every person, without a hidden row cap.
  async function people(cycleId, { q = '', grantSubteams = null, scope = null, cycle = null } = {}) {
    let apps;
    if (kit.mode === 'memory') {
      apps = memApps().filter((a) => a.cycleId === cycleId && a.erasedAt == null);
    } else {
      const s = await kit.sql();
      const r = await s`SELECT id, cycle_id, section, email, ts, updated, name, cornell, subteam, year, source, files, review, edit_version, review_version, jsonb_build_object('subteam', answers->'subteam') AS answers
        FROM recruit_applications WHERE cycle_id = ${cycleId} AND erased_at IS NULL ORDER BY ts DESC`;
    ...
    if (granted) apps = apps.filter((a) => granted.includes(a.subteam) || (Array.isArray(a.answers?.subteam) && a.answers.subteam.some((v) => granted.includes(v))));
    if (scope) apps = apps.filter((a) => scope.has(a.id));
    ...
    const rows = [...byEmail.values()].filter((p) => !needle || String(p.name).toLowerCase().includes(needle) || p.email.includes(needle)).sort((a, b) => b.last - a.last || a.email.localeCompare(b.email));
    const bySection = {};
    for (const a of apps) bySection[sectionOf(a)] = (bySection[sectionOf(a)] || 0) + 1;
    return { rows, total: rows.length, counts: { people: byEmail.size, bySection } };
  }
  ...
  async function list(cycle, opts = {}) {
    ...
    const questions = sectionsFor(cyc)[o.section]?.form?.questions || [];
    const previewKey = questions.find((q) => ['long', 'longfile'].includes(q.type))?.key || questions.find((q) => q.type === 'short' && !COLUMN_KEYS.includes(q.key))?.key || null;
    ...
    if (kit.mode === 'memory') {
      ...
      const inBase = (a) => a.cycleId === cycleId && (!o.scope || o.scope.includes(a.id)) && (!o.grantSubteams || (o.grantSubteams.includes(a.subteam) || (Array.isArray(a.answers?.subteam) && a.answers.subteam.some((v) => o.grantSubteams.includes(v)))));
      ...
      if (o.undecided) rows = rows.filter((a) => !a.subteam); else if (o.subteam) rows = rows.filter((a) => (o.subteam.includes(a.subteam) || (Array.isArray(a.answers?.subteam) && a.answers.subteam.some((v) => o.subteam.includes(v)))));
      if (o.noYear) rows = rows.filter((a) => !a.year); else if (o.year) rows = rows.filter((a) => (a.year === o.year || (Array.isArray(a.answers?.year) && a.answers.year.includes(o.year))));
      ...
      if (o.emails) rows = rows.filter((a) => o.emails.includes(a.email));
      ...
      return { rows: page.map((a) => ({ ...listRow(project(a)), preview: previewKey ? String(a.answers?.[previewKey] ?? '').slice(0, 160) : '' })), next, total, counts: { byStage, bySection } };
    }
    ...
    if (o.grantSubteams) base` AND (a.subteam = ANY(${o.grantSubteams}) OR (jsonb_typeof(a.answers->'subteam') = 'array' AND a.answers->'subteam' ?| ${o.grantSubteams}))`;
    const where = kit.build().append(base);
    if (o.emails) where` AND a.email = ANY(${o.emails})`;
    if (o.section) where` AND a.section = ${o.section}`;
    if (o.stage) where` AND a.stage = ${o.stage}`;
    if (o.undecided) where` AND a.subteam = ''`; else if (o.subteam) where` AND (a.subteam = ANY(${o.subteam}) OR (jsonb_typeof(a.answers->'subteam') = 'array' AND a.answers->'subteam' ?| ${o.subteam}))`;
    if (o.noYear) where` AND a.year IS NULL`; else if (o.year) where` AND (a.year = ${o.year} OR (jsonb_typeof(a.answers->'year') = 'array' AND a.answers->'year' ? ${o.year}))`;
    ...
    rowsQ`SELECT a.id, a.cycle_id, a.section, a.email, a.name, a.ts, a.updated, a.cornell, a.subteam, a.year, a.source, a.stage, a.stage_at, a.outcome, a.tags, a.files, a.edit_version, a.review_version,
      left(COALESCE(a.answers->>${previewKey}::text, ''), 160) AS preview`;
...
async function listRoute(rq, kit) {
  const qy = rq.query;
  const people = kit.people ? await kit.people.result({ ...rq, query: { flagged: qy.flagged, comments: qy.comments } }) : { rows: [] };
  const summaries = new Map(people.rows.map((p) => [p.email, p]));
  const out = await kit.apps.list(rq.cycle, {
    emails: qy.flagged === '1' || qy.comments === '1' ? [...summaries.keys()] : null,
  ...
  const sourceRows = people.rows.flatMap((p) => Object.values(p.sections || {}).filter((s) => !qy.section || s.section === qy.section));
  const facets = Object.fromEntries(['year', 'subteam'].map((key) => [key, [...new Set(sourceRows.flatMap((s) => String(s[key] || '').split('; ').filter(Boolean)))].sort()]));
  return { status: 200, body: { ...out, facets, rows: out.rows.map((r) => { const p = summaries.get(r.email); return { ...r, flagged: p?.flagged || false, comments: p?.comments || 0, reviewVersion: p?.reviewVersion || 0, extras: extras[r.id] || {} }; }) } };
}
...
async function detailRoute(rq, kit) {
  ...
  const history = await visibleHistory(kit, rq.me, await kit.apps.history(app.email, app.id));
  return {
    status: 200,
    body: {
...
async function csvRoute(rq, kit) {
  ...
  await kit.csv(rq.res, rows, columns, `cupi-${rq.cycle.term || rq.cycle.name}-${key}-${new Date(kit.now()).toISOString().slice(0, 10)}`);
  return undefined;
}
...
async function patchRoute(rq, kit) {
  ...
  const changed = new Set([...Object.keys(fields), ...Object.keys(fields.answers || {})]);
  const questions = formForRow(sectionsFor(rq.cycle), cur).questions.filter((q) => ['name', 'email'].includes(q.key) || (changed.has(q.key) && q.type !== 'file'));
  const values = { ...cur.answers, name: cur.name, email: cur.email, year: cur.answers?.year ?? cur.year, subteam: cur.answers?.subteam ?? cur.subteam, ...fields, ...(fields.answers || {}) };
  const teamQuestion = questions.find((q) => q.key === 'subteam' && q.type === 'single');
  if (fields.subteam !== undefined && teamQuestion && !fields.answers?.subteam) values.subteam = teamQuestion.options?.find((v) => v === values.subteam || slug(v) === slug(values.subteam)) || values.subteam;
  const checked = validateAnswers({ questions: questions.map((q) => q.type === 'longfile' && cur.files?.some((f) => f.question === q.key) ? { ...q, required: false } : q) }, { answers: values }, rq.cycle);
  if (checked.error) throw fail(checked.status || 400, checked.error);
  if (changed.has('name')) fields.name = checked.columns.name;
  for (const key of ['year', 'subteam']) if (changed.has(key)) fields[key] = checked.columns[key];
  if (Object.keys(checked.answers).length) fields.answers = { ...(fields.answers || {}), ...checked.answers };
  const result = await kit.once(requestId, rq.me.email, async () => {
...
async function serveFile(rq, kit) {
  ...
  if (rq.role !== 'admin') {
    ...
    const cycle = await kit.cycles.get(owner.cycleId);
    const scope = await scopeFor(kit, { cycle, me: rq.me, role: roleOf(rq.me, grant), grant });
    if (scope && !scope.has(owner.id)) throw fail(404, 'No such file');
  }
...
async function applicantRoute(rq, kit) {
  ...
  if (rq.role !== 'admin') {
    ...
    for (const app of out.applications) {
      const grant = await kit.roles.grantFor(app.cycleId, rq.me.email);
      if (!grant?.roles?.length) continue;
      const cycle = await kit.cycles.get(app.cycleId);
      const scope = await scopeFor(kit, { cycle, me: rq.me, role: roleOf(rq.me, grant), grant });
      if (!scope || scope.has(app.id)) visible.push(app);
    }
    out.applications = visible;
    // Global identity metadata may also contain facts from inaccessible cycles.
    const last = visible[0];
    out.applicant = last ? { email: last.email, name: last.name, cornell: last.cornell, applications: visible.length } : null;
  }
...
const pendingEntry = (entry, reason, form) => ({
  ...
  files: (entry.files || []).map((f, i) => ({ ...f, url: `/api/recruit/queue/${entry.id}/file?index=${i}` })),
...
async function placeRoute(rq, kit) {
  ...
  const result = await kit.once(requestId, rq.me.email, async () => {
    const out = await kit.intake.commit({ ...entry, section: body.section || entry.section || 'interest', confirmUpdate: body.confirmUpdate === true || Boolean(entry.confirmUpdate) }, journal, null, { cycleId: cycle.id, override: true, source: entry.cycleId ? 'form' : 'orphan' });
    if (out.outcome === 'held') throw fail(409, 'This form is full');
...
async function queueFile(rq, kit) {
  const journal = await kit.intake.journal();
  const entry = await journal.getEntry(rq.params.receipt);
  const indexed = Array.isArray(entry?.files);
  const index = rq.query.index === undefined ? 0 : Number(rq.query.index);
  if (!Number.isInteger(index) || index < 0) throw fail(404, 'No saved attachment');
  const f = indexed ? entry.files[index] : index === 0 && entry?.fileSize ? {name:entry.fileName,type:entry.fileType,size:entry.fileSize} : null;
  if (!f) throw fail(404, 'No saved attachment');
  const data = await journal.getFile(entry.id, indexed ? index : null);
  if (!data || data.length !== f.size) throw fail(503, 'The saved attachment is temporarily unavailable');
  kit.files.serve(rq.res, { ...f, data });
  return undefined;
}

async function storageCheck(rq, kit) {
  const database = { ok: false, mode: kit.mode, transaction: false };
  const backup = { ok: false };
  try {
    if (kit.mode === 'memory') database.ok = true;
    else {
      const sql = await kit.sql();
      await sql.transaction(async (s) => {
        await s`SET TRANSACTION READ ONLY`;
        const result = await s`SELECT 1 AS ok`;
        if (Number(result.rows[0]?.ok) !== 1) throw new Error('Database check returned no result');
      });
      database.ok = true;
      database.transaction = true;
    }
  } catch (e) { /* report only the failing service, never connection details */ }
  try {
    const journal = await kit.intake.journal();
    backup.ok = (await journal.check())?.ok === true;
  } catch (e) { /* the journal owns its diagnostic write/read/cleanup */ }
  const ok = database.ok && backup.ok;
  const failed = [!database.ok && 'Database transactions', !backup.ok && 'Private submission backup storage'].filter(Boolean).join(' and ');
  return { status: ok ? 200 : 503, body: { ok, database, backup, ...(!ok ? { error: `${failed} could not be verified. Try again shortly.` } : {}) } };
}
