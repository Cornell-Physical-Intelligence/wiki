...
import { validateAnswers, fixedFormFor, DEFAULT_STAGES, FILE_TYPES, MAX_FILE, EMAIL_RE, isCornell, slug } from '../fixed-form.js';
...
const appFromPg = (r) => ({
  ...
  subteam: r.subteam || '', year: r.year || null, source: r.source || 'form', formVersion: Number(r.form_version || 0),
...
function makeFacade(kit) {
  ...
  async function findByEmail(cycleId, email) {
    if (kit.mode === 'memory') return project(memApps().find((a) => a.cycleId === cycleId && a.email === email));
    const s = await kit.sql();
    const r = await s`SELECT * FROM recruit_applications WHERE cycle_id = ${cycleId} AND email = ${email}`;
    return r.rows[0] ? appFromPg(r.rows[0]) : null;
  }

  async function count(cycleId) {
    if (kit.mode === 'memory') return memApps().filter((a) => a.cycleId === cycleId).length;
    const s = await kit.sql();
  ...
  async function commitIntake(cycle, entry, journal, originalFile = null, { confirmUpdate = Boolean(entry.confirmUpdate), override = false, source = 'form' } = {}) {
    ...
    const existing = await findByEmail(cycle.id, email);
    ...
    const row = {
      id: existing?.id || 'in-' + stripId(rid), cycleId: cycle.id, email, ts: existing?.ts || entry.ts, updated: entry.ts,
    ...
    if (kit.mode === 'memory') {
      ...
      const m = kit.mem;
      const current = memApps().find((a) => a.cycleId === cycle.id && a.email === email);
      ...
      const saved = memApps().find((a) => a.cycleId === cycle.id && a.email === email);
    ...
    const r = await s`WITH prior AS (
      ...
      INSERT INTO recruit_applications (id, cycle_id, email, ts, updated, name, cornell, subteam, year, source, form_version, answers, files, stage, stage_at, stage_history, ip_hash, receipt_id)
      SELECT ${row.id}, ${row.cycleId}, ${row.email}, ${row.ts}, ${row.updated}, ${row.name}, ${row.cornell}, ${row.subteam}, ${row.year}, ${row.source}, ${row.formVersion},
      ...
      ON CONFLICT (cycle_id, email) DO UPDATE SET name = EXCLUDED.name, subteam = EXCLUDED.subteam, updated = EXCLUDED.updated, ip_hash = EXCLUDED.ip_hash,
    ...
    const saved = outcome === 'saved' ? await findByEmail(cycle.id, email) : null;
    return { outcome, existing, row: saved || project(row), inserted };
  }

  // Ported verbatim from interest.js updateReview, against recruit_applications.
  async function updateReview(id, { flag, comment, deleteComment }) {
    ...
    if (kit.mode === 'memory') {
      const row = memApps().find((r) => r.id === id);
      if (!row) return failure(null);
      const comments = row.review?.comments || [];
      const deleted = row.review?.deletedCommentIds || [];
      if (deleteComment && !comments.some((c) => c.id === deleteComment)) return failure(project(row));
      if (comment && (comments.length >= 200 || comments.length + deleted.length >= 1000 || deleted.includes(comment.id) || comments.some((c) => c.id === comment.id))) return failure(project(row));
    ...
    const result = deleteComment
      ? await s`UPDATE recruit_applications SET
      ...
      ? await s`UPDATE recruit_applications SET
        ...
        RETURNING *`
      : await s`UPDATE recruit_applications SET review = review || ${JSON.stringify(flag)}::jsonb,
          review_version = review_version + 1 WHERE id = ${id} RETURNING *`;
    if (result.rows[0]) return { row: appFromPg(result.rows[0]) };
    const current = await s`SELECT * FROM recruit_applications WHERE id = ${id}`;
    return failure(current.rows[0] ? appFromPg(current.rows[0]) : null);
  }
  ...
  async function create(cycle, { name, email, cornell, subteam, year, answers, files = [], source = 'admin', by, now = kit.now() }) {
    const id = 'in-' + Math.random().toString(36).slice(2, 10) + now.toString(36);
    const stage = firstStageOf(cycle);
    const row = { id, cycleId: cycle.id, email, ts: now, updated: now, name, cornell: Boolean(cornell), subteam: subteam || '', year: year || null, source, formVersion: cycle.formVersion || 0, answers: answers || {}, files, stage, stageAt: now, stageHistory: [{ stage, at: now, by }], outcome: null, decision: {}, tags: [], review: {}, reviewVersion: 0, editVersion: 0, onboardedAt: null, erasedAt: null, receiptId: null, ipHash: '' };
    if (kit.mode === 'memory') {
      if (memApps().some((a) => a.cycleId === cycle.id && a.email === email)) return null;
      memApps().push(row);
    ...
    const r = await s`WITH written AS (
      INSERT INTO recruit_applications (id, cycle_id, email, ts, updated, name, cornell, subteam, year, source, form_version, answers, files, stage, stage_at, stage_history)
      VALUES (${id}, ${cycle.id}, ${email}, ${now}, ${now}, ${name}, ${row.cornell}, ${row.subteam}, ${row.year}, ${source}, ${row.formVersion}, ${JSON.stringify(row.answers)}::jsonb, ${JSON.stringify(files)}::jsonb, ${stage}, ${now}, ${JSON.stringify(row.stageHistory)}::jsonb)
      ON CONFLICT (cycle_id, email) DO NOTHING RETURNING *
    ), applicant AS (
      INSERT INTO recruit_applicants (email, name, cornell, first_seen, last_seen, applications)
  ...
  async function history(email, exceptId) {
    if (kit.mode === 'memory') {
      ...
      return memApps().filter((a) => a.email === email && a.id !== exceptId).sort((a, b) => b.ts - a.ts).slice(0, 20)
        .map((a) => ({ id: a.id, cycleId: a.cycleId, cycleName: cycles.get(a.cycleId) || a.cycleId, stage: a.stage, outcome: a.outcome, ts: a.ts }));
    }
    const s = await kit.sql();
    const r = await s`SELECT a.id, a.cycle_id, c.name AS cycle_name, a.stage, a.outcome, a.ts FROM recruit_applications a LEFT JOIN recruit_cycles c ON c.id = a.cycle_id
      WHERE a.email = ${email} AND a.id <> ${exceptId} ORDER BY a.ts DESC LIMIT 20`;
    return r.rows.map((x) => ({ id: x.id, cycleId: x.cycle_id, cycleName: x.cycle_name || x.cycle_id, stage: x.stage, outcome: x.outcome || null, ts: Number(x.ts) }));
  }
  ...
  const listRow = (a) => ({
    id: a.id, cycleId: a.cycleId, email: a.email, name: a.name, ts: a.ts, updated: a.updated, cornell: Boolean(a.cornell), subteam: a.subteam || '', year: a.year || null,
    source: a.source, stage: a.stage, stageAt: a.stageAt || 0, outcome: a.outcome || null, tags: a.tags || [], flagged: a.review?.flagged === true, comments: (a.review?.comments || []).length,
  ...
  function listOptions(cycle, cyc, opts) {
    ...
    return {
      ...
      flagged: opts.flagged === '1' || opts.flagged === true, unscored: (opts.unscored === '1' || opts.unscored === true) && hasScores,
  ...
  async function list(cycle, opts = {}) {
    ...
    if (kit.mode === 'memory') {
      ...
      if (o.flagged) rows = rows.filter((a) => a.review?.flagged === true);
      ...
      return { rows: page.map((a) => listRow(project(a))), next, total, counts: { byStage } };
    }
    ...
    if (o.flagged) where` AND COALESCE((a.review->>'flagged')::boolean, false)`;
    ...
    rowsQ`SELECT a.id, a.cycle_id, a.email, a.name, a.ts, a.updated, a.cornell, a.subteam, a.year, a.source, a.stage, a.stage_at, a.outcome, a.tags, a.files, a.edit_version, a.review_version,
      COALESCE((a.review->>'flagged')::boolean, false) AS flagged, jsonb_array_length(COALESCE(a.review->'comments', '[]'::jsonb)) AS comments,
    ...
    return {
      rows: pageRows.map((x) => ({
        id: x.id, cycleId: x.cycle_id, email: x.email, name: x.name, ts: Number(x.ts), updated: Number(x.updated), cornell: Boolean(x.cornell), subteam: x.subteam || '', year: x.year || null,
        source: x.source, stage: x.stage, stageAt: Number(x.stage_at || 0), outcome: x.outcome || null, tags: asArray(x.tags), flagged: Boolean(x.flagged), comments: Number(x.comments || 0),
      ...
      next, total, counts: { byStage },
    };
  }

  return {
    get, findByEmail, count, ids, commitIntake, updateReview, move, setDecision, patch, remove, toLegacyRow, legacyRows, create, addFile, fileOwner,
    applicant, history, eraseApplicant, purgePage, receiptOutcome, recordOutcome, listRow, list,
  };
}
...
async function listRoute(rq, kit) {
  ...
  const out = await kit.apps.list(rq.cycle, {
    q: qy.q, stage: qy.stage, subteam: qy.subteam, year: qy.year, tag: qy.tag, flagged: qy.flagged, outcome: qy.outcome, source: qy.source,
...
async function detailRoute(rq, kit) {
  ...
  const form = rq.cycle.formVersion > 0 && kit.tables.has('recruit_forms') ? await formQuestions(kit, rq.cycle) : { questions: fixedFormFor(rq.cycle).questions };
  const application = { ...app };
  if (!lead && !stagesOf(rq.cycle).some((s) => s.key === app.stage && s.kind !== 'open')) application.decision = { ...app.decision, reason: undefined };
  let audit = undefined;
  if (lead) {
  ...
  return {
    ...
    body: {
      ...
      scores: extras.scores ?? [], assignments: extras.assignments ?? [], bookings: extras.bookings ?? [],
      ...(lead ? { mail: extras.mail ?? [], audit } : {}),
    },
  };
}

async function formQuestions(kit, cycle) {
  if (kit.mode === 'memory') {
    const f = (kit.mem.forms || []).find((x) => x.cycleId === cycle.id && Number(x.version) === cycle.formVersion);
    return { questions: f?.doc?.questions || fixedFormFor(cycle).questions };
  }
  const s = await kit.sql();
  const r = await s`SELECT doc FROM recruit_forms WHERE cycle_id = ${cycle.id} AND version = ${cycle.formVersion}`;
  return { questions: asObject(r.rows[0]?.doc).questions || fixedFormFor(cycle).questions };
}

function validated(cycle, body) {
  const v = validateAnswers(fixedFormFor(cycle), body, cycle);
...
async function createRoute(rq, kit) {
  ...
  if (Array.isArray(body.import)) {
    ...
    const result = await kit.once(requestId, rq.me.email, async () => {
      ...
      for (const item of body.import) {
        let v;
        try { v = validated(rq.cycle, item); } catch (e) { skipped.push({ email: String(item?.email || '').slice(0, 200), reason: e.error || 'Invalid' }); continue; }
        const app = await kit.apps.create(rq.cycle, { ...v.columns, answers: v.answers, source: 'import', by: rq.me.email, now });
  ...
  const v = validated(rq.cycle, body);
  const result = await kit.once(requestId, rq.me.email, async () => {
    const app = await kit.apps.create(rq.cycle, { ...v.columns, answers: v.answers, source: 'admin', by: rq.me.email, now });
...
async function patchRoute(rq, kit) {
  ...
  if (fields.year !== undefined && fields.year !== null && fields.year !== '' && !['Freshman', 'Sophomore', 'Junior', 'Senior', 'Grad'].includes(fields.year)) throw fail(400, 'Choose Freshman, Sophomore, Junior, Senior, or Grad for year');
  const result = await kit.once(requestId, rq.me.email, async () => {
...
async function flagRoute(rq, kit) {
  ...
  const app = await kit.apps.get(rq.cycle, rq.params.app);
  if (!app || (rq.scope && !rq.scope.has(app.id))) throw fail(404, 'No such application');
  const out = await kit.apps.updateReview(app.id, { flag: { flagged: body.flagged, flaggedBy: rq.me.email, flaggedName: rq.me.name || rq.me.email, flaggedAt: kit.now() } });
  if (out.error) throw fail(out.status, out.error);
  return { status: 200, body: { row: kit.apps.toLegacyRow(out.row), application: out.row }, audit: { kind: 'app.flag', detail: { flagged: body.flagged } } };
}
...
async function commentRoute(rq, kit) {
  ...
  const app = await kit.apps.get(rq.cycle, rq.params.app);
  if (!app || (rq.scope && !rq.scope.has(app.id))) throw fail(404, 'No such application');
  const out = await kit.apps.updateReview(app.id, { comment: { id: body.id, text: body.text.trim(), by: rq.me.email, name: rq.me.name || rq.me.email, ts: kit.now() } });
  if (out.error) throw fail(out.status, out.error);
  return { status: 200, body: { row: kit.apps.toLegacyRow(out.row), application: out.row }, audit: { kind: 'comment.post', detail: { commentId: body.id } } };
}

async function deleteCommentRoute(rq, kit) {
  const app = await kit.apps.get(rq.cycle, rq.params.app);
  if (!app || (rq.scope && !rq.scope.has(app.id))) throw fail(404, 'No such application');
  const existing = (app.review?.comments || []).find((c) => c.id === rq.params.comment);
  if (existing && existing.by !== rq.me.email && !isLead(rq.role)) throw fail(403, 'Not allowed in this cycle');
  const out = await kit.apps.updateReview(app.id, { deleteComment: rq.params.comment });
  if (out.error) throw fail(out.status, out.error);
  return { status: 200, body: { row: kit.apps.toLegacyRow(out.row), application: out.row }, audit: { kind: 'comment.delete', detail: { commentId: rq.params.comment } } };
}
...
const pendingEntry = (entry, reason, cycleId) => ({
  id: entry.id, receivedAt: entry.ts, reason, cycleId: cycleId ?? entry.cycleId ?? null, name: entry.name, email: entry.email,
  subteam: entry.subteam, year: entry.year, project: entry.project ?? entry.answers?.project ?? '',
...
async function queueRoute(rq, kit) {
  ...
  for (const entry of entries) {
    ...
    if (reason) pending.push(pendingEntry(entry, reason));
  }
...
export default {
  ...
  schema: [
    ...
    `CREATE TABLE IF NOT EXISTS recruit_applications (id text PRIMARY KEY, cycle_id text NOT NULL, email text NOT NULL, ts bigint NOT NULL, updated bigint NOT NULL, name text NOT NULL, cornell boolean NOT NULL DEFAULT false, subteam text NOT NULL DEFAULT '', year text, source text NOT NULL DEFAULT 'form', form_version int NOT NULL DEFAULT 0, answers jsonb NOT NULL DEFAULT '{}'::jsonb, files jsonb NOT NULL DEFAULT '[]'::jsonb, stage text NOT NULL DEFAULT 'applied', stage_at bigint NOT NULL DEFAULT 0, stage_history jsonb NOT NULL DEFAULT '[]'::jsonb, outcome text, decision jsonb NOT NULL DEFAULT '{}'::jsonb, tags jsonb NOT NULL DEFAULT '[]'::jsonb, review jsonb NOT NULL DEFAULT '{}'::jsonb, review_version bigint NOT NULL DEFAULT 0, edit_version bigint NOT NULL DEFAULT 0, ip_hash text NOT NULL DEFAULT '', onboarded_at bigint, erased_at bigint, receipt_id text, UNIQUE (cycle_id, email))`,
  ...
  routes: [
    ...
    { method: 'PATCH', path: '/cycles/:cycle/applications/:app/review', access: 'reviewer', scoped: true, mutates: true, cap: 32000, handler: flagRoute },
    { method: 'POST', path: '/cycles/:cycle/applications/:app/comments', access: 'reviewer', scoped: true, mutates: true, cap: 32000, handler: commentRoute },
    { method: 'DELETE', path: '/cycles/:cycle/applications/:app/comments/:comment', access: 'reviewer', scoped: true, mutates: true, handler: deleteCommentRoute },
  ...
  hooks: {},
  collect: {},
  auditKinds: ['app.create', 'app.edit', 'app.delete', 'app.erase', 'app.flag', 'comment.post', 'comment.delete', 'stage', 'queue.place'],
};
