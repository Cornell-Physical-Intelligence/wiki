...
export function sectionsFor(cycle) {
  ...
  for (const key of sectionKeysFor(cycle)) {
    ...
    const s = { thanks: DEFAULT_THANKS[key] || '', ...def, ...(isObject(saved[key]) ? saved[key] : {}) };
    ...
    if (!saved[key]?.form && subteams.length) for (const qq of s.form.questions) if (qq.key === 'subteam' && qq.type === 'single') qq.options = [...subteams];
    s.open = s.open === true;
...
export function publicSections(cycle) {
  return Object.entries(sectionsFor(cycle)).map(([key, s]) => ({ key, title: s.title, description: s.description, thanks: s.thanks, submitLabel: s.submitLabel || 'Send', successLabel: s.successLabel || 'Sent', replace: s.replace, open: s.open, form: { questions: s.form.questions.map(publicQuestion) } }));
}
...
// identity questions; every form only needs a name and an email.
export function validateSite(next, cycle = null) {
  ...
  for (const key of keys) {
    if (['__proto__', 'prototype', 'constructor'].includes(key)) throw fail(400, 'Choose a different form key');
    if (key === 'people' && !Object.hasOwn(current, key)) throw fail(400, 'The form key people is reserved; use people-form');
    ...
    const base = existing || { title: clip(s.title, 80), description: '', thanks: '', open: false, capacity: 0, notify: true, notifyTo: [], replace: true, form: { questions: defaultQuestions() } };
    const cur = { submitLabel: base.submitLabel || 'Send', successLabel: base.successLabel || 'Sent', title: base.title, description: base.description, thanks: String(base.thanks ?? '').trim().slice(0, 300), open: base.open, capacity: base.capacity || 0, notify: base.notify !== false, notifyTo: cleanRecipients(base.notifyTo), replace: base.replace !== false, form: { questions: base.form.questions.map(cleanQuestion) } };
    if (s !== undefined) {
