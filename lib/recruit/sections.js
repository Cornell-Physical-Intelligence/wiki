...
export function landingFor(cycle, sections = sectionsFor(cycle)) {
  ...
  if (SECTION_KEYS.includes(chosen) && sections[chosen]?.open) return chosen;
  return SECTION_KEYS.find((key) => sections[key]?.open) || null;
}

// Edits from the Form view: partial per section, merged over what the cycle
// has now, plus the /apply choice. The interest form keeps the website's
// fixed questions; the other two only need a name and an email.
export function validateSite(next, cycle = null) {
  ...
  if (next.landing !== undefined) {
    landing = next.landing === null || next.landing === '' ? null : String(next.landing);
    if (landing !== null && !SECTION_KEYS.includes(landing)) throw fail(400, 'The form shown at /apply must be the interest form, coffee chats, or the application');
  }
  const out = {};
  for (const key of SECTION_KEYS) {
    const s = given[key];
    const cur = { title: current[key].title, description: current[key].description, open: current[key].open, capacity: current[key].capacity || 0, notify: current[key].notify !== false, replace: current[key].replace !== false, form: { questions: current[key].form.questions.map(cleanQuestion) } };
    if (s !== undefined) {
      ...
      if (s.capacity !== undefined) {
        ...
        if (!Number.isInteger(n) || n < 0 || n > 100000) throw fail(400, `${SECTION_LABELS[key]}: stop accepting after a whole number of responses, up to 100,000`);
        cur.capacity = n;
      }
