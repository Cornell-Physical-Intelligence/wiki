...
export function validateSite(next, cycle = null) {
  ...
  for (const key of keys) {
    ...
    const base = existing || { title: clip(s.title, 80), description: '', open: false, capacity: 0, notify: true, replace: true, form: { questions: defaultQuestions() } };
    const cur = { title: base.title, description: base.description, open: base.open, capacity: base.capacity || 0, notify: base.notify !== false, replace: base.replace !== false, form: { questions: base.form.questions.map(cleanQuestion) } };
    if (s !== undefined) {
      ...
      if (s.form !== undefined) {
        let v;
        try { v = validateFormDoc({ questions: Array.isArray(s.form?.questions) ? s.form.questions : [] }, { required: key === FIXED_FORM ? SYSTEM_KEYS : ['name', 'email'] }); }
