...
// Who a form emails: its own list, else the wiki's default recipients.
const EMAIL = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
export const NOTIFY_MAX = 10;
const cleanRecipients = (list) => (Array.isArray(list) ? [...new Set(list.map((e) => clip(e, 120).toLowerCase()).filter((e) => EMAIL.test(e)))].slice(0, NOTIFY_MAX) : []);
  ...
  for (const key of keys) {
    ...
    if (s !== undefined) {
      ...
      if (s.notifyTo !== undefined) {
        ...
        const badOne = list.find((e) => !EMAIL.test(e));
