...
export function publicSections(cycle) {
  return Object.entries(sectionsFor(cycle)).map(([key, s]) => ({ key, title: s.title, description: s.description, thanks: s.thanks, open: s.open, form: { questions: s.form.questions.map(publicQuestion) } }));
}
...
export function validateSite(next, cycle = null) {
  ...
  for (const key of keys) {
    ...
    const cur = { title: base.title, description: base.description, thanks: String(base.thanks ?? '').trim().slice(0, 300), open: base.open, capacity: base.capacity || 0, notify: base.notify !== false, notifyTo: cleanRecipients(base.notifyTo), replace: base.replace !== false, form: { questions: base.form.questions.map(cleanQuestion) } };
    if (s !== undefined) {
