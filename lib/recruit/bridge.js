...
import { sectionsFor, hasSystemKeys } from './sections.js';
...
export function createIntakeBridge(kit) {
  ...
  const bridge = {
    ...
    async target() {
      ...
      if (!t) return null;
      // The old route posts the fixed interest form, so it is open only while
      // the cycle has that form, open, with every fixed question still on it.
      // Every other rule is the form's own: its cap, repeats, recipients.
      let form = null;
      try { const cycle = await kit.cycles.get(t.cycleId); form = cycle ? sectionsFor(cycle).interest || null : null; } catch (e) { form = null; }
      const closed = !form || !form.open || !hasSystemKeys(form);
      return {
        cycleId: t.cycleId, section: 'interest', closed, formVersion: t.formVersion, perIpHour: t.perIpHour, perDay: t.perDay,
        capacity: form ? form.capacity : 0, notify: Boolean(form && form.notify && form.notifyTo.length), notifyTo: form ? form.notifyTo : [], confirmUpdate: form ? form.replace !== false : true,
      };
    },
    ...
    async commit(entry, journal, file, target) {
      // Both old and new receipts can be drained by the legacy queue reader.
      // Only an unsectioned legacy receipt belongs to the interest adapter.
      if (entry.section) return kit.intake.commit(entry, journal, file, target);
      const cycle = await kit.cycles.get(target.cycleId);
      if (!cycle) throw new Error('No such cycle');
      if (!sectionsFor(cycle).interest) throw new Error('This cycle has no interest form');
      entry.section = 'interest';
      const result = await kit.apps.commitIntake(cycle, entry, journal, file, {
        confirmUpdate: Boolean(entry.confirmUpdate) && target.confirmUpdate !== false,
      ...
      return {
        ...
        inserted: Boolean(result.inserted), notify: target.notify !== false, to: Array.isArray(target.notifyTo) ? target.notifyTo : [],
