...
export function createIntakeBridge(kit) {
  ...
  const bridge = {
    ...
    async target() {
      ...
      if (!t) return null;
      return {
        cycleId: t.cycleId, formVersion: t.formVersion, perIpHour: t.perIpHour, perDay: t.perDay,
        capacity: t.capacity, notify: t.notify, confirmUpdate: t.confirmUpdate,
      };
    },

    async find(target, email) {
      return legacy(await kit.apps.findByEmail(target.cycleId, String(email || '').toLowerCase()));
    },

    async count(target) {
      return kit.apps.count(target.cycleId);
    },
