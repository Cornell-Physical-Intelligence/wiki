...
export const QUESTION_TYPES = ['short', 'long', 'email', 'single', 'multi', 'checkbox', 'link', 'file', 'longfile'];
...
export function validateAnswers(formDoc, body, cycle = null) {
  ...
  if (src.files != null && !isPlainObject(src.files)) return err('The attachments could not be read. Attach the files again.');
  const uploadKeys = new Set((form.questions || []).filter((q) => q.type === 'file' || q.type === 'longfile').map((q) => q.key));
  if (Object.entries(filesGiven).some(([key, file]) => file != null && !uploadKeys.has(key)) || (src.file != null && !uploadKeys.has('file'))) {
    return err('The form changed. Reload it and attach your files again.');
  }
  ...
  for (const q of form.questions || []) {
    ...
    const raw = key === 'subteam' && !src.answers && q.type === 'single' ? subteam : valueOf(key);
    const present = raw !== undefined && raw !== null && raw !== '';
    // A file question takes a file; a long-text-plus-file question takes a
    // file and then reads its text below like any long answer.
    if (q.type === 'file' || q.type === 'longfile') {
      const f = fileOf(key);
      if (f != null && (!isPlainObject(f) || typeof f.data !== 'string' || !f.data)) return err(`Attach the file for ${label} again; it could not be read`);
      if (f && typeof f === 'object' && f.data) {
        const data = String(f.data);
        if (!/^[A-Za-z0-9+/=]+$/.test(data)) return err('The file did not decode');
        const buf = Buffer.from(data, 'base64');
        if (!buf.length || buf.toString('base64').replace(/=+$/, '') !== data.replace(/=+$/, '')) return err('The file did not decode');
        if (buf.length) {
    ...
    switch (q.type) {
      case 'short': case 'long': case 'longfile': case 'email': {
        const v = q.type === 'long' || q.type === 'longfile' ? String(raw ?? '').trim().slice(0, q.max || 1000) : text(raw, q.max || (q.type === 'email' ? 200 : 200));
        if (q.type === 'email' && v && !EMAIL_RE.test(v.toLowerCase())) return err(`That email for ${label} does not look right`);
        // Required, for text with a file, is satisfied by either one.
        if (q.required && !v && !(q.type === 'longfile' && files.some((x) => x.question === key))) return err(`${label} is required`);
  ...
  if (Object.entries(filesGiven).some(([key, file]) => file != null && !files.some((f) => f.question === key))) return err('The form changed. Reload it and attach your files again.');
  for (const key of ['year', 'subteam']) {
    const v = answers[key];
    columns[key] = Array.isArray(v) ? v.join('; ') : v == null ? (key === 'year' ? null : '') : String(v);
    if (!src.answers) delete answers[key]; // the old flat POST retains its column-only shape
  }
  if (files.reduce((n, f) => n + f.size, 0) > MAX_FILE) return err('Attachments together are capped at 2.5 MB', 413);
  if (Buffer.byteLength(JSON.stringify(answers)) > 400000) return err('The combined answers are too long', 413);
...
export function validateFormDoc(doc, { required = SYSTEM_KEYS } = {}) {
  ...
  return { sections, questions: questions.map((q) => ['name', 'email'].includes(q.key) ? { ...q, required: true } : q) };
}
