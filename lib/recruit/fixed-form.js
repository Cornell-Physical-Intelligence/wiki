  ...
  for (const q of form.questions || []) {
    ...
    if (key === 'year') {
      if (columns.hasYear && raw != null && raw !== '' && !(q.options || YEARS).includes(raw)) {
        return err('Choose Freshman, Sophomore, Junior, Senior, or Grad for year');
      }
...
export function validateFormDoc(doc) {
  ...
  for (const q of questions) {
    ...
    if (system.has(q.key) && system.get(q.key).type !== q.type) throw err(`The ${q.key} question cannot change type`);
  ...
  for (const key of SYSTEM_KEYS) if (!seen.has(key)) throw err(`The ${key} question cannot be removed`);
