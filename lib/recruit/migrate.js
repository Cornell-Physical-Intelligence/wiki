...
const appFromLegacy = (r, cycleId, source = 'migrated') => ({
  ...
  name: r.name || '', cornell: Boolean(r.cornell), subteam: r.subteam || '', year: r.year || null, source, formVersion: 0,
...
export async function importLegacy(kit, step = {}) {
  ...
  const now = kit.now();
  const defaults = kit.cycles.defaultDoc({ id: LIVE_CYCLE, name: 'Interest list', term: 'Rolling' });
  ...
  if (step.step === 'live') {
    if (kit.mode === 'memory') {
      ...
      if (!m.cycles.some((c) => c.id === LIVE_CYCLE)) {
        m.cycles.push({ id: LIVE_CYCLE, version: 1, status: 'open', name: 'Interest list', term: 'Rolling', opensAt: null, closesAt: null, closedAt: null, created: now, createdBy: by, updated: now, updatedBy: by, formVersion: 0, doc: defaults, legacy: { source: 'live' } });
      }
      ...
      await s`INSERT INTO recruit_cycles (id, status, name, term, created, updated, created_by, updated_by, legacy, doc)
        VALUES (${LIVE_CYCLE}, 'open', 'Interest list', 'Rolling', ${now}, ${now}, ${by}, ${by}, '{"source":"live"}'::jsonb, ${JSON.stringify(defaults)}::jsonb)
