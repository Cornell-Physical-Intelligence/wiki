...
// Filter cross-cycle links with the same grants and scope as detail routes.
export async function visibleHistory(kit, me, rows) {
  if (me?.role === 'admin') return rows;
  const cache = new Map();
  const out = [];
  for (const row of rows) {
    if (!cache.has(row.cycleId)) {
      const cycle = await kit.cycles.get(row.cycleId);
      const grant = cycle ? await kit.roles.grantFor(cycle.id, me.email) : null;
      const role = roleOf(me, grant);
      cache.set(row.cycleId, role ? { scope: await scopeFor(kit, { cycle, me, role, grant }) } : null);
    }
    const access = cache.get(row.cycleId);
    if (access && (!access.scope || access.scope.has(row.id))) out.push(row);
  }
  return out;
}
