...
// The signed-in member and when their session ends, or null for a missing,
// tampered, or expired cookie.
export function readSessionInfo(cookieHeader) {
...
export function readSession(cookieHeader) {
  return readSessionInfo(cookieHeader)?.email ?? null;
}

// Sessions last 30 days and renew on use: a request in the second half of a
// session gets a fresh cookie, so an active member is never signed out
// mid-session, while 30 days away still ends it. Returns the Set-Cookie
// value to send, or null when the session is still young.
export function renewSession(info) {
  if (!info || info.exp - Date.now() > (DAYS30 * 1000) / 2) return null;
  return sessionCookie(makeSession(info.email));
}
