...
export function createIntakeJournal({
  ...
  return {
    ...
    async append(entry, file, { perDay = 300, perIpHour = 5 } = {}) {
      ...
      if (day.length >= perDay || hour.length >= perIpHour) {
        throw Object.assign(new Error('Too many submissions. Try again later or email cuphysint@cornell.edu.'), { status: 429 });
      }
      const sdk = await loadBlob();
      if (Array.isArray(file)) {
        for (const [index, attachment] of file.entries()) {
          await sdk.put(`${PREFIX}files/${entry.id}/${index}`, attachment.data, { ...options, contentType: attachment.type, allowOverwrite: false });
        }
      } else if (file) {
    ...
    async getFile(id, index = null) {
      if (!validReceipt(id)) return null;
      if (index !== null && (!Number.isInteger(index) || index < 0 || index >= 60)) return null;
      const sdk = await loadBlob();
      const result = await sdk.get(`${PREFIX}files/${id}${index === null ? '' : '/' + index}`, { access: 'private', useCache: false });
      if (!result || result.statusCode !== 200 || !result.stream) return null;
      return Buffer.from(await new Response(result.stream).arrayBuffer());
    },
    // Erase and retention purge: drop the record and its attachment. Best
    // effort; a missing blob is already forgotten.
    async forget(id) {
      if (!enabled() || !validReceipt(id)) return false;
      const sdk = await loadBlob();
      const day = dateOf(Number(id.split('-')[1]));
      const record = (await blobs(`${PREFIX}records/${day}/`)).find((b) => b.pathname.endsWith(`/${id}.json`));
      const targets = [record?.url, (await blobs(`${PREFIX}files/${id}`)).map((b) => b.url)].flat().filter(Boolean);
      if (targets.length) await sdk.del(targets);
      return true;
    },
