...
const ADMIN_OPS = new Set(['addMembers', 'setRole', 'removeUser', 'purgePage', 'setEmailSettings', 'deleteInterest', 'purgeInterest']);
...
const OPS = {
  ...
  // Interest-list rows are written by the public /api/interest endpoint, never
  // through ops. Admins can only remove them; the API caller deletes any files
  // the removed rows referenced (returned via _result), like the purge sweep.
  deleteInterest(state, { id }, actor) {
    const list = state.interest || [];
    const i = list.findIndex((r) => r.id === id);
    if (i < 0) return 'No such submission';
    const [row] = list.splice(i, 1);
    log(state, { kind: 'interest-remove', by: actor, who: row.email });
    state._result = { fileIds: row.fileId ? [row.fileId] : [] };
  },

  purgeInterest(state, args, actor) {
    const list = state.interest || [];
    const fileIds = list.map((r) => r.fileId).filter(Boolean);
    log(state, { kind: 'interest-purge', by: actor, count: list.length });
    state.interest = [];
    state._result = { fileIds };
  },
