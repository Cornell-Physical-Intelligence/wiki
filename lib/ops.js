...
const ADMIN_OPS = new Set(['addMembers', 'resendInvite', 'revokeInvite', 'setRole', 'removeUser', 'purgePage']);
...
const OPS = {
  createPage(state, { title, section, parent, body, tags, summary }, actor) {
    ...
    const now = Date.now();
    state.pages.push({
      id, title, section: String(section || 'projects'), parent: parent || null,
  ...
  savePage(state, { id, title, body, summary, section, tags }, actor) {
  ...
  toggleTask(state, { id, n }, actor) {
    ...
    if (!p) return 'No such page';
    let count = -1;
    p.body = p.body.replace(/^(\s*(?:[-*]|\d+[.)])\s+\[)( |x|X)(\])/gm, (m, pre, st, post) => {
      count++;
      return count === n ? pre + (st.trim() ? ' ' : 'x') + post : m;
    });
  ...
  restoreRev(state, { id, revIdx }, actor) {
    const p = page(state, id);
  ...
  movePage(state, { id, section, parent }) {
    const p = page(state, id);
    if (!p) return 'No such page';
    p.section = String(section || p.section);
    p.parent = parent && page(state, parent) && parent !== id ? parent : null;
  },
  ...
  addComment(state, { pageId, text, cid }, actor) {
    if (!page(state, pageId)) return 'No such page';
    text = String(text || '').trim().slice(0, 4000);
    if (!text) return 'Empty comment';
    state.comments.push({ id: cid || uid('c'), pageId, by: actor, ts: Date.now(), text });
    log(state, { kind: 'comment', by: actor, pageId });
    // @first-name / @netid notify that member's inbox.
    const seen = new Set();
    for (const m of text.matchAll(/@([a-z0-9.]+)/gi)) {
      const tok = m[1].toLowerCase();
      const u = state.users.find((x) => x.email.split('@')[0] === tok || x.name.split(/\s+/)[0].toLowerCase() === tok);
      if (u && u.email !== actor && !seen.has(u.email)) { seen.add(u.email); log(state, { kind: 'mention', by: actor, who: u.email, pageId }); }
    }
  },

  deleteComment(state, { id }, actor) {
    const i = state.comments.findIndex((c) => c.id === id);
    if (i < 0) return 'No such comment';
    const me = user(state, actor);
    if (state.comments[i].by !== actor && me?.role !== 'admin') return 'Not allowed';
  ...
  addMembers(state, { emails, role }, actor) {
    ...
    for (const raw of Array.isArray(emails) ? emails : []) {
      ...
      const u = {
        ...
        invitedAt: Date.now(), invitedBy: actor, inviteCode: inviteCode(),
      };
      state.users.push(u);
      log(state, { kind: 'invite', by: actor, who: email });
      results.push({ email, ok: true, code: u.inviteCode });
    }
    state._result = results;
  },

  resendInvite(state, { email }, actor) {
    const u = user(state, email);
    if (!u || u.status !== 'invited') return 'No pending invite';
    u.inviteCode = inviteCode();
    u.invitedAt = Date.now();
    log(state, { kind: 'invite-resend', by: actor, who: email });
    state._result = { email, code: u.inviteCode };
  },

  revokeInvite(state, { email }, actor) {
    const i = state.users.findIndex((u) => u.email === email && u.status === 'invited');
    if (i < 0) return 'No pending invite';
    state.users.splice(i, 1);
    log(state, { kind: 'invite-revoke', by: actor, who: email });
  },
