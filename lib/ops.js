...
export function cleanPreferences(prefs) {
  ...
  if (Number.isFinite(src.inboxReadAt)) clean.inboxReadAt = src.inboxReadAt;
  for (const k of ['navHidden', 'spellcheck']) if (typeof src[k] === 'boolean') clean[k] = src[k];
...
const OPS = {
  createPage(state, { title, section, parent, body, tags, summary, requestId }, actor) {
    const createRequestId = typeof requestId === 'string' && /^[a-zA-Z0-9-]{12,80}$/.test(requestId) ? requestId : null;
    if (createRequestId) {
      const prior = state.pages.find((p) => p.createRequestId === createRequestId && p.owner === actor);
      if (prior) { state._result = { id: prior.id }; return; }
      if (state.trash.some((p) => p.createRequestId === createRequestId && p.owner === actor)) return 'This page was created and then moved to Trash. Restore it from Trash to continue.';
    }
  ...
  savePage(state, { id, title, body, summary, section, tags, baseUpdated }, actor) {
    ...
    const now = Math.max(Date.now(), (p.updated || 0) + 1);
    const newTitle = title !== undefined ? String(title).trim().slice(0, 90) : p.title;
    if (!newTitle) return 'Title required';
    // Metadata is authored content too. A stale title/section-only save must
    // not undo another editor's rename or move just because the body is equal.
    const changed = (body !== undefined && String(body) !== p.body) || newTitle !== p.title || (section !== undefined && String(section) !== p.section);
    const tagsChanged = tags !== undefined && JSON.stringify(tags) !== JSON.stringify(p.tags);
    if (Number.isFinite(baseUpdated) && p.updated !== baseUpdated && (changed || tagsChanged)) {
  ...
  movePage(state, { id, section }, actor) {
    const p = page(state, id);
    if (!p) return 'No such page';
    if (String(section || p.section) === p.section && !p.parent) return;
    p.section = String(section || p.section);
    p.parent = null;
    p.updated = Math.max(Date.now(), (p.updated || 0) + 1);
    p.updatedBy = actor;
  ...
  setEmailSettings(state, { key, from, name }, actor) {
    ...
    if (!k && !addr && !cur.key && !cur.oauth) return 'Nothing to save yet: connect Resend, or paste an API key or a From address';
  ...
  setPrefs(state, { prefs }, actor) {
    ...
    state.prefs[actor] = { ...(state.prefs[actor] || {}), ...cleanPreferences(prefs) };
  },
...
export function applyOp(state, op, args, actor, role) {
  if (typeof op !== 'string' || !Object.hasOwn(OPS, op) || typeof OPS[op] !== 'function') return { error: 'Unknown op' };
...
// One-time content heal: deployments seeded before the crab joined the
// welcome page get the drawing added. Returns false (no write) once any
// crab reference exists, so this can run on every cold start forever.
export function healWelcomeCrab(state) {
  const w = (state.pages || []).find((p) => p.id === 'welcome');
  if (!w || typeof w.body !== 'string') return false;
  if (w.body.includes('/welcome-crab.png') || w.body.includes('att:att-crab')) return false;
  const line = '![Crab on the beach](/welcome-crab.png "The apply-page crab. Physical intelligence, demonstrated.")';
  const cut = w.body.indexOf('\n\n');
  w.body = cut === -1 ? w.body + '\n\n' + line : w.body.slice(0, cut) + '\n\n' + line + w.body.slice(cut);
  const ts = Date.now();
  w.updated = ts;
  w.updatedBy = w.owner || w.updatedBy;
  if (Array.isArray(w.revs)) w.revs.push({ ts, by: w.updatedBy, summary: 'Added the beach crab drawing', body: w.body });
  return state;
}
