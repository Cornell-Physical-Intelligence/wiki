...
export async function handleAgentContext(req, res, path, { origin }) {
  ...
  const browserContext = new URL(req.url, origin).pathname === '/context';
  for (const [key, value] of Object.entries({
    'cache-control': 'no-store, max-age=0',
    'referrer-policy': 'no-referrer',
    'x-content-type-options': 'nosniff',
    'link': `<${origin}/context>; rel="alternate"; type="text/html", <${origin}/llms-full.txt>; rel="alternate"; type="text/plain", <${origin}/llms.txt>; rel="describedby"; type="text/plain"`,
  ...
  if (browserContext) {
    // An ordinary HTML document for browser tools that cannot open text files.
    // No app boot, session checks, inline scripts, or wiki markup execution.
    res.statusCode = 200;
    res.setHeader('content-type', 'text/html; charset=utf-8');
    res.setHeader('content-security-policy', "default-src 'none'; style-src 'unsafe-inline'");
    const index = state.pages.map((p) => `<li><a href="/context?page=${encodeURIComponent(p.id)}">${escapeHtml(p.title)}</a></li>`).join('');
    const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex, follow"><title>CUPI Wiki — public context, no login required</title><style>body{max-width:960px;margin:32px auto;padding:0 20px;font:16px/1.6 system-ui}pre{white-space:pre-wrap;overflow-wrap:anywhere;font:inherit}a{color:inherit}</style></head><body><main><h1>${requestedPage === null ? 'All wiki pages' : 'Wiki page'} — public context</h1><p>No login required. Recruitment applications are separate and are not included.</p><p><a href="/context">All pages in one document</a> · <a href="/llms-full.txt">Plain-text Markdown</a></p><nav aria-label="Wiki page index"><ul>${index}</ul></nav><pre>${escapeHtml(context)}</pre></main></body></html>`;
  ...
  // The app replaces #app after boot; the shell's discovery note remains
  // outside it in accessibility snapshots, alongside head and HTTP links.
