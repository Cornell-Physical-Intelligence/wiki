...
async function pg() {
  ...
  if (!pgReady) pgReady = (async () => {
    ...
    if (!r.rows.length) {
      await query`INSERT INTO wiki_state (id, version, state) VALUES (1, 1, ${JSON.stringify(seedState())}) ON CONFLICT DO NOTHING`;
    }
    sql = query;
...
export async function getMember(email) {
  if (useMemory) {
    memLoad();
    return { member: mem.state.users.find((u) => u.email === email) || null, version: mem.version };
  }
  const s = await pg();
