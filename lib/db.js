// Storage. Production: Postgres (Vercel Marketplace injects POSTGRES_URL).
...
async function pg() {
  if (!sql) {
    const { sql: s } = await import('@vercel/postgres');
    sql = s;
...
export async function putFile(f) {
  ...
  await s`INSERT INTO wiki_files (id, name, type, size, by, ts, data)
          VALUES (${f.id}, ${f.name}, ${f.type}, ${f.size}, ${f.by}, ${f.ts}, ${f.data})`;
}

export async function getFile(id) {
  if (useMemory) { memLoad(); return mem.files.get(id) || null; }
  const s = await pg();
  const r = await s`SELECT * FROM wiki_files WHERE id = ${id}`;
...
export async function listFiles() {
  if (useMemory) { memLoad(); return [...mem.files.values()].map(({ data, ...m }) => m); }
  const s = await pg();
  const r = await s`SELECT id, name, type, size, by, ts FROM wiki_files ORDER BY ts DESC`;
