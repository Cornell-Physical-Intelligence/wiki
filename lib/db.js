...
async function pg() {
  ...
  if (!pgReady) pgReady = (async () => {
    ...
    // A transaction must keep one checked-out connection. In particular,
    // recruitment capacity checks and their inserts share a row lock.
    query.transaction = async (run) => {
      const client = await pool.connect();
      const sql = (strings, ...vals) => client.sql(strings, ...vals);
      try {
