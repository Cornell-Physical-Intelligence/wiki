...
- Recruitment lives in `lib/recruit/` (server modules behind one registry) and `src/client/recruit-*.js` (client modules behind `RECRUIT.register`). Add capabilities as new modules; do not grow `lib/interest.js` or the legacy interest panel. See README "Applications (recruitment)".
